...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    do {
        ...
        if (isset($_POST["btngrava"])) {
            ...
            if (empty($c_escolha)) {
                $c_erro = "Nenhuma unidade básica foi selecionada!!";
                break;
            } else {
            ...
            if (
                empty($c_nome) || (empty($c_rg)) || (empty($c_endereco)) || (empty($c_cartao))
                || (empty($c_bairro)) || (empty($c_naturalidade))
            ) {

                $c_erro = "Campos nome, endereço, bairro, RG, Cartão SUS e Naturalidade devem ser preenchidos!!";
                break;
            }
            ...
            $c_sql = 'SELECT *  FROM cadastro where sus_cpf=' . $_SESSION['cpf'];
            ...
            if ($_SESSION['c_tipo'] == "Eleitor") {
                $c_sql = "Insert into cadastro (categoria,candidato,sus_nome,sus_cpf,sus_escolaridade,sus_rg,sus_datanasc,sus_ender
                ,sus_bairro, sus_naturalidade, sus_unidade, sus_cartaosus)" .
                    "Value ('$c_categoria', '$c_tipocadastro', '$c_nome', '$c_cpf', '$c_escolaridade', '$c_rg','$d_datanasc' 
                , '$c_endereco','$c_bairro', '$c_naturalidade', '$c_unidade', '$c_cartao')";
            } else {
                $c_foto = $_SESSION['c_nomefoto'];

                $c_sql = "Insert into cadastro (categoria,candidato,sus_nome,sus_cpf,sus_escolaridade,sus_rg,sus_datanasc,sus_ender
                ,sus_bairro, sus_naturalidade, sus_unidade, sus_cartaosus, apresentacao, foto)" .
                    "Value ('$c_categoria', '$c_tipocadastro', '$c_nome', '$c_cpf', '$c_escolaridade', '$c_rg','$d_datanasc' 
                , '$c_endereco','$c_bairro', '$c_naturalidade', '$c_unidade', '$c_cartao', '$c_apresentacao', '$c_foto')";
            }
            if (!$registro2) {
                $result = $conection->query($c_sql);
